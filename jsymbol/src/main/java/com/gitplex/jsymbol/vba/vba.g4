...
functionStmt :
	(visibility WS)? (STATIC WS)? FUNCTION WS? ambiguousIdentifier typeHint? (WS? argList)? (WS? asTypeClause)? ignoreInsideBraces 'End Function'
;
//endOfStatement
//	block?
//	END_FUNCTION
...
endOfStatement1 : (endOfLine | WS? COLON WS?);
...
subStmt : 
	(visibility WS)? (STATIC WS)? SUB WS? ambiguousIdentifier (WS? argList)? (WS? asTypeClause)? ignoreInsideBraces
	;
